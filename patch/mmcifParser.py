...
"""
    ...
    This file ePMV_git/patch/mmcifParser.py is part of ePMV.
...
%d %s in chain %s"%(len(endRes), endData, chain.id))
    ...
    def parse_MMCIF_CELL(self):
        ...
        self.mol.cellLength = [ float(a), float(b), float(c) ]
        self.mol.cellAngles = [ float(alpha), float(beta), float(gamma) ]
    ...
    def buildBondsmol(self,):
        self.mol.buildBondsByDistance()
        
    def buildBonds(self):
        mmCIF_dict = self.mmCIF_dict
        if "_geom_bond_atom_site_label_1" in mmCIF_dict :
            self.buildBondscif()
        else :
            self.buildBondsmol()
...
if __name__ == '__main__':
    ...
    print "Done"
#from MolKit.mmcifParser import MMCIFParser;parser = MMCIFParser(filename="/Users/ludo/Downloads/MLC166.cif")