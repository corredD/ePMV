...
"""
    ...
    This file ePMV_git/pmv_dev/colorCommands_3x.py is part of ePMV.
...
#
# $Header: /opt/cvs/python/packages/share1.5/Pmv/colorCommands.py,v 1.159 2010/11/01 17:18:29 autin Exp $
#
# $Id: colorCommands.py,v 1.159 2010/11/01 17:18:29 autin Exp $
#
"""
...
#            for n,c in map(None,nodes,colors):
        ...
        for g in geomsToColor:
            if len(colors)==1 or len(colors)!=len(atms):
                for a in atms:
                    if g not in a.colors: continue
                    a.colors[g] = tuple( colors[0] )
            else:
                for a, c in zip(atms, colors):
                    if g not in a.colors: continue
...
#                    print (a.colors[g])
    ...
    def __call__(self, nodes, colors=[(1.,1.,1.),],
        ...
        if not type(geomsToColor) in [list, tuple]:
            return 'ERROR'
        geomsToColor = [x for x in geomsToColor if x not in [' ', '']]
        if not len(geomsToColor):
            return 'ERROR'
        status = self.doitWrapper(*(nodes, colors, geomsToColor), **kw)
        return status
...
##                               self.undoCmdsString )
        ...
        for g in geomsToColor:
            ...
            atmsWithGeom = atms.get(lambda x, geom=g: geom in x.colors)
    ...
    def getChildrenGeomsName(self, mol):
        ...
        childGeomsName = []
        for geomName in list(geomC.geoms.keys()):
    ...
    def dismiss_cb(self):
        if 'chooseColor' in self.cmdForms:
...
class ColorFromPalette(ColorCommand, MVAtomICOM):
    ...
    def onAddCmdToViewer(self):
        ...
        if 'color' not in self.vf.commands:
...
##             self.warningMsg("ERROR: No geometry to color")
    ...
    def __call__(self, nodes, geomsToColor='all', **kw):
        ...
        status = self.doitWrapper(*(nodes, geomsToColor), **kw)
        return status
...
#        print (nodes,type(nodes))
        colors = self.palette.lookup(nodes.parent.parent)
#        colors = self.palette.lookup(nodes.findType(Chain))
        return colors
    
    def buildFormDescr(self, formName):
        idf = super(ColorByChain, self).buildFormDescr(formName)
...
class ColorByDomains(ColorFromPalette):
    """The colorByChain command allows the user to color the given geometries representing the given nodes by chain. A different color is assigned to each chain.
    \nPackage : Pmv
    \nModule  : colorCommands
    \nClass   : ColorByChain
...
##             self.warningMsg("ERROR: No geometry to color")
    ...
    def onAddCmdToViewer(self):
        ...
        from mglutil.util.defaultPalettes import MolColors, Rainbow, RainbowSortedKey
        c = 'Color palette domains number'
        
        self.palette = paletteClass(
            'MolColors', MolColors, readonly=0, info=c,
            lookupFunction = lambda x, length = len(RainbowSortedKey):\
            x.domainFragmentNumber%length, sortedkeys = RainbowSortedKey)
...
#        print nodes
        ...
        if not hasattr(mol,"hasDomains") or self.forcedomain:
            domains= self.getDomains(mol,self.method)
            if domains < 0 :
                return
        if mol.hasDomains :
            #need to color each geom in geoms
            #use chain colorplette
            #self.colorByChains.palette.Map([0,1,2])
            #fin the atom domain attribution
            nodes = nodes.findType(Atom)
            colors = self.palette.lookup(nodes)
            return colors
...
##             self.warningMsg("ERROR: No geometry to color")
    ...
    def buildFormDescr(self, formName):
        idf = super(ColorByMolecule, self).buildFormDescr(formName)
        idf.insert(2, {'widgetType':tkinter.Checkbutton,
                    'name':'Carbons',
                    'defaultValue':1,
...
class ColorByProperties(ColorCommand):
    ...
    def onAddCmdToViewer(self):
        ...
        if 'color' not in self.vf.commands:
            self.vf.loadCommand('colorCommands', 'color', 'Pmv',
                                topCommand=0)
        if 'saveSet' not in self.vf.commands:
...
current selection cannot be colored using this function."
    ...
    def loadCM_cb(self, event=None):
        ...
        selcm = cbb.get()
        cbb.clear()
        cbb._list.setlist(list(self.vf.colorMaps.keys()))
    ...
    def entryValidate(self, text):
        if not text in list(self.vf.colorMaps.keys()):
            return Pmw.PARTIAL
        else:
            return 1
     ...
     FunctionButton, ListChooser
        if formName == 'default':
            ...
            if len(self.properties)>0 :
                ...
                cmNames = list(self.vf.colorMaps.keys())
                if len(cmNames): 
 ...
 and maxi",
    ...
    def guiCallback(self):
        ...
        if 'byProp' in self.cmdForms:
            ...
            cbb = ebn['colormap']['widget']
            cbb.clear()
            cms = list(self.vf.colorMaps.keys())
       ...
       # Show the colormapeditor
        ...
        del val['editmap']
        val['log']=1
        if 'level' in val: del val['level']
        val['redraw']=1
        val['propertyLevel'] = self.propertyLevel
        self.doitWrapper(*(self.selection, geomsToColor, property), **val)
...
#        #nodes = nodes.findType(self.vf.selectionLevel)
        ...
        if colormap is None:
          ...
          and colormap in self.vf.colorMaps:
            colormap = self.vf.colorMaps[colormap]
#        print (colormap,isinstance(colormap, ColorMap))
...
## """)
                ...
                for n in nodes:
                    ...
                    for a in atms:
                        if not carbonsOnly or a.chemElem == "C":
                            if gName in a.colors:
                                a.colors[gName] = tuple(selectioncol[i][:3])
                            if gName in a.opacities:
                                newOpac = selectioncol[i][3]
    ...
    def __call__(self, nodes, geomsToColor, property, propertyLevel=None,
        ...
        status = self.doitWrapper(*(nodes, geomsToColor, property), **kw)
        return status
...
##             self.warningMsg("ERROR: No geometry to color")
        ...
        val['redraw'] = 1
        if 'level' in val: del val['level']
        self.doitWrapper(*(self.selection,geomsToColor), **val)
    ...
    def doit(self, nodes,  geomsToColor,
        ...
        if atms is None:
            return "ERROR"
        # Get the colormap
        if type(colormap) is str and \
           colormap in self.vf.colorMaps:
            colormap = self.vf.colorMaps[colormap]
...
class RainbowColor(ColorFromPalette):
  ...
  """  
    ...
    def doit(self, nodes, geomsToColor='all', propertyLevel=None,
        ...
        atoms._uniqNumber = list(range(1, len(atoms)+1))  
...
commandList = [
    ...
    {'name':'colorByDomains', 'cmd':ColorByDomains(), 'gui':None}, 
