...
"""
    ...
    This file ePMV_git/epmvGui_26.py is part of ePMV.
...
class ApplyTransformationPanel(uiadaptor):
    ...
    def setup(self,epmv,id=None):
        ...
        self.block = True
        self.epmv = epmv
        self.title = "Apply Modified Coordinates"#+self.mol.name
...
class BindGeomToMol(uiadaptor):
    ...
    def setup(self,epmv,id=None):
        ...
        self.block = True
        self.epmv = epmv
        self.title = "Bind Polygon to current Molecule"#+self.mol.name
#        self.SetTitle(self.title)
        witdh=350
        self.h=130
        self.w=300
    ...
    def bind(self,*args):
        ...
        #update g
        mesh=self.epmv.helper.getMesh(obj)
        #obj need to be triangulate
        #self.epmv.helper.triangulate(mesh)
        f,v,n = self.epmv.helper.DecomposeMesh(obj,edit=False,copy=False,tri=True,transform=True)
        #update dejaVu geometry
#        poly = self.epmv.helper.createsNmesh("b_"+name,v,None,f)
#        self.epmv.helper.addObjectToScene(None,poly[0])
        g.Set(vertices=v,faces=f)
...
class Parameter_epmvGUI(uiadaptor):
    def setup(self,epmv,id=None):
        ...
        self.BTN = self._addElemt(name="OK",action=self.SetPreferences,width=50,
                          type="button")
    ...
    def setupLayout(self):
        ...
        for key in self.EPMVOPTIONS:
            if self.EPMVOPTIONS[key]["type"] == "pullMenu":
                label = self._addElemt(
                    label=self.epmv.keywords[key]["name"],
                    width=120)
                self._layout.append([label,self.EPMVOPTIONS[key],])
            else :
...
class epmvGui(uiadaptor):
    ...
    __about__+="""
ePMV by Ludovic Autin,Graham Johnson,Michel Sanner.
...
#                       {"id": id+6, "name":"BiCylinders&c&","action":self.optionCyl}],
        ...
        self.LABEL_ID.append(self._addElemt(label="to a PDB file OR enter a 4 digit ID (e.g. 1crn):",
                                            width=120))#0
        self.LABEL_ID.append(self._addElemt(label="",width=1))#1
        self.LABEL_ID.append(self._addElemt(label="Current selection :",width=50))#2
        self.LABEL_ID.append(self._addElemt(label="Add selection set using string or",width=120))#3
        self.LABEL_ID.append(self._addElemt(label="Scheme:",width=50))#4
        self.LABEL_ID.append(self._addElemt(label="to load a Data file",width=50))#5
        self.LABEL_ID.append(self._addElemt(label="to Current Selection and play below:",width=120))#6
        self.LABEL_ID.append(self._addElemt(label="PMV-Python scripts/commands",width=50))#7
        self.LABEL_ID.append(self._addElemt(label="Molecular Representations",width=50))#8
        self.LABEL_ID.append(self._addElemt(label="Apply",width=30))#9
        self.LABEL_ID.append(self._addElemt(label="a Selection Set",width=50))#10
        self.LABEL_ID.append(self._addElemt(label="atoms in the Selection Set",width=120))#11
        self.LABEL_ID.append(self._addElemt(label="or",width=10))#12
        self.LABEL_ID.append(self._addElemt(label="or",width=10))#13
        self.LABEL_ID.append(self._addElemt(label=":",width=10))#14
        self.LABEL_ID.append(self._addElemt(label="Or choose a custom color",width=100))#15
        self.LABEL_ID.append(self._addElemt(label="Customs Geometry",width=100))#16
        ...
        self.SAVE_BTN = self._addElemt(name="Save as PDB",width=75,height=10,
...
#        print "id del button", id-1
        ...
        self.COMB_BOX = {"mol":self._addElemt(name="Current",value=[],
                         ...
                         "col":self._addElemt(name="Color",
                                    value=self.colSchem,
                                    width=160,height=10,action=self.color,
                                    variable=self._col,
                         ...
                         "datatype":self._addElemt(name="DataTypes",
                                    value=self.datatype,
                                    width=25,height=10,action=None,
                         ...
                         "cgeom":self._addElemt(name="Customs Geom:",
                                    ...
                                    width=200,height=10,action=self.updateCGeomDisplay,
...
#        self._layout.append([self.LABEL_ID[2],self.COMB_BOX["mol"]])
        ...
        elemFrame=[]
        elemFrame.append([self.LABEL_ID[2],self.COMB_BOX["mol"],self.SAVE_BTN])        
...
#        self.epmv.testNumberOfAtoms(self.mv.Mols[-1])
        ...
        #get the starting representation
        self.updateViewer()
...
#            sys.stderr.write('before Select and isoContour')            
            ...
            #print trajname
            if mname == "" and "IsoSurface" not in list(self.mv.iMolData.keys()):
...
#            print sel,mname, mol , "del"
    ...
    def dsPoints(self,*args):
        ...
        if obj is None :
            obj = self.epmv._PointCloudObject(mol.name+"_cloud",
...
#        self.mv.displayLines(selection)
        ...
        for ch in mol.chains :
            ...
            if obj is None :
                ...
                obj = self.epmv._PointCloudObject(ch.full_name()+"_cloud",
...
#        self.updateViewer()
    ...
    def dsSS(self,*args):        
        ...
        if not hasattr(self.mv, 'secondarystructureset'):
            ...
            self.mv.computeSecondaryStructure(
                mol.name, molModes={'%s'%mol.name:mod}, topCommand=0)
            self.mv.extrudeSecondaryStructure(mol.name, topCommand=0, 
                                              log=0, display=0)
    ...
    def dsCMS(self,*args):
        ...
        if name not in mol.geomContainer.geoms :
            geom=self.epmv.coarseMolSurface(selection,[gridsize,gridsize,gridsize],
            ...
            self.epmv.helper.addObjectToScene(self.epmv.helper.getCurrentScene(),
                                          obj[0],parent=parent)
            self.mv.colorByResidueType(selection, [name], log=0)
            obj=obj[0]
        else :
    ...
    def dsMSMS(self,*args):
        ...
        if name in mol.geomContainer.geoms :
            ...
            self.mv.colorAtomsUsingDG(sel, [name], log=1)
        return True
...
#            self.updateViewer()
    ...
    def check_update(self,*args):
        ...
        if True in doit :
            ...
            res = self.drawQuestion("update?",question=msg)
            if res :
                res = self.drawQuestion("backup?",question="Do you want to backup the current version?")
    ...
    def drawBIOMT(self,*args):
        ...
        if mol is None :
            return
        geomname=self.drawInputQuestion(title="Build biological unit",
            question="Which geometry do you want to use?",
            callback=None)
        if not geomname :
            return
        ...
        if not hasattr(mol, "biomat"):
            mat = self.epmv.parse_PDB_BIOMT(mol.parser.allLines)
            mol.biomat = mat
        ...
        for symOpNum in mol.biomat : 
            geom_inst = self.epmv.helper.getObject(geomname+str(symOpNum))
            if geom_inst is None :
                m = mol.biomat[symOpNum]
                m.append([0.,0.,0.,1.])
                geom_inst = self.epmv.helper.newInstance(geomname+str(symOpNum),
                        geom,matrice=mol.biomat[symOpNum],
                    parent = geomInstParent,material=None)
